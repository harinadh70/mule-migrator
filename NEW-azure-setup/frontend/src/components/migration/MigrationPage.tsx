...
export default function MigrationPage() {
  ...
  const [uploadedFiles, setUploadedFiles] = useState<Array<{name: string; content: string; size: number; path: string}>>([]);
    ...
    try {
      const result = await createMutation.mutateAsync({
        ...
        config: {
          ...
          llmModel: aiEnhancement ? selectedModel : "",
      ...
      showToast({ type: "error", title: "Failed to start migration", message: msg });
    }
  }
  ...
  function handleRetry() {
    ...
    retryMutation.mutate(id, {
      onSuccess: () => {
        showToast({ type: "success", title: "Migration re-queued", message: "The migration has been re-queued for processing." });
      },
      onError: (err: any) => {
        showToast({ type: "error", title: "Retry failed", message: err?.message || "Could not retry migration." });
      },
    });
  }

  function handleAgentClick(_agentType: AgentType) {
  ...
  const _isFailed = migration?.status === "failed";
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex items-start justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            {migration.status === "completed" && (
              <GradientButton
                ...
                onClick={() => navigate(`/validate/${migration.id}`)}
              >
                Test & Validate
              </GradientButton>
            )}
        ...
        {/* Retry button for failed/cancelled/stuck migrations */}
        {isViewMode && (migration?.status === "failed" || migration?.status === "cancelled" || migration?.status === "running") && (
          <GradientButton
            variant="primary"
            size="sm"
          ...
          >
            {retryMutation.isPending ? "Retrying..." : "Retry Migration"}
          </GradientButton>
        )}
      </div>
                  ...
                  {aiEnhancement && (
                    <div className="mt-3 flex gap-2">
                      ...
                      >
                        ...
                        GitHub Copilot
                        {aiProvider === "github_copilot" && (
                          <span className="rounded bg-[#0070AD]/20 px-1.5 py-0.5 text-[9px] font-bold dark:bg-[#12ABDB]/20">{selectedModel}</span>
                        )}
                      </button>
                      <button
                        type="button"
                  ...
                  {aiEnhancement && (
                    <div className="mt-3">
                      ...
                      <select
                        ...
                        className="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm dark:border-white/[0.1] dark:bg-white/[0.04] dark:text-gray-200"
                      >
                        <optgroup label="OpenAI">
                          ...
                          <option value="o3-mini">O3 Mini (Pro)</option>
                          <option value="o4-mini">O4 Mini (Pro)</option>
                        </optgroup>
                        ...
                        <optgroup label="Meta">
                          ...
                          <option value="Llama-4-Maverick-17B-128E-Instruct-FP8">Llama 4 Maverick</option>
                        </optgroup>
                        <optgroup label="Mistral (Pro)">
